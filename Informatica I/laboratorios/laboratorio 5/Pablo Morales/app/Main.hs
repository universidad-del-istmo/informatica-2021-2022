...
import Prelude (Int, Float, Bool(..), Show, (+), (-), (==), (||), (*), (>), (&&), (**), floor, fst, snd, div, mod, drop, take, undefined)
...
-- elem i Nil = 0
-- elem 0 (Cons x xs) = x
-- elem i (Cons x xs) = elem (i - 1) xs
-- elemAgg (Cons x Nil) = x

elemAgg (Cons x Nil) = x 

elemAgregador (resultado, i) x = 
    if i == 0 
        then m 
        else n 
...
elemAux i xs = fold (elemAgregador) (Nil, i) xs

elem i xs = elemAgg (fst (elemAux i (reverse xs)))
...
-- Definición: función map alterna = map'
map :: (Int -> Int) -> Lista -> Lista
map _ Nil = Nil
map n (Cons x xs) = Cons (n x) (map n xs)

map' :: Int -> (Int -> t -> b) -> Lista -> t -> (Lista, b)
map' n estado x xs = (Cons n x, estado n xs)


foldm :: ((t1 -> t2) -> t1 -> t2) -> t3 -> t1 -> t1 -> t2
foldm map' estado x xs = map' (foldm map' estado xs) x 
