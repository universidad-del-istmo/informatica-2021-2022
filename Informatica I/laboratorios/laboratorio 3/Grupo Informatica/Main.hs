...
import Prelude (undefined, Float, Int, abs, pi, sqrt, (<), sin, cos, pi, exp, log, asin, acos, atan, sinh, cosh, asinh, acosh, atanh, (<=), (/), (+), (^), (*), (-), div, mod, (==), (/=), (>), (>=), even, (&&), (||), Bool (False, True))
...
estaAdentro (cx, cy) radio (x, y) = sqrt ((x - cx) ^ 2 + (y - cy) ^ 2) < radio
...
esEquilatero (x1, y1) (x2, y2) (x3, y3) = abs ((sqrt ((x2 - x1) ^ 2 + (y2 - y1) ^ 2) + sqrt ((x3 - x2) ^ 2 + (y3 - y2) ^ 2) + sqrt ((x3 - x1) ^ 2 + (y3 - y1) ^ 2)) - (sqrt ((x2 - x1) ^ 2 + (y2 - y1) ^ 2) * 3)) <= 0.001
...
rotacion angulo punto =  transformacionLineal ((cos angulo, -sin angulo), (sin angulo, cos angulo)) punto
