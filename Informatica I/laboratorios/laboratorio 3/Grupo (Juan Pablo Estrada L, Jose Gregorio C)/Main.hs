...
import Prelude(Bool, Float, undefined, (==), (+), sqrt, (-), (^), (&&), (*), (>=), (<=), cos, sin, pi, (/), abs, )
...
esEquilatero (x1, y1) (x2, y2) (x3, y3) = 
       abs (sqrt ((x2 - x1) ^ 2 + (y2 - y1) ^ 2) - sqrt ((x3 - x2) ^ 2 + (y3 - y2) ^ 2)) <= 0.001 && 
        abs (sqrt ((x3 - x2) ^ 2 + (y3 - y2) ^ 2) - sqrt ((x1 - x3) ^ 2 + (y1 - y3) ^ 2)) <= 0.001 && 
       abs  (sqrt ((x2 - x1) ^ 2 + (y2 - y1) ^ 2) - sqrt ((x1 - x3) ^ 2 + (y1 - y3) ^ 2)) <= 0.001
...
rotacion angulo punto = 
      transformacionLineal ((m11, m12), (m21, m22)) punto 
