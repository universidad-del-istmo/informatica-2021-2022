...
fold agg 0 Nil = 0
fold agg 0 (Cons x xs) =
    agg (fold agg 0 xs) x

mcdAgg estado x = mcd x estado 
mcdfold xs = fold mcdAgg 0 xs
...
fold2 agg cero Nil = cero
fold2 agg cero (Cons x xs) =
    agg (fold2 agg cero xs) x

mcmAgregador estado x = mcm2 x estado 

mcmfold2 xs = fold2 mcmAgregador 1 xs


-- pregutnas impares 1, 3, 5
main = undefined
