...
module Main where
import Prelude ( Int, Float, Show, Bool(..), (+), (-), (*), mod, (&&), (==), undefined)
import Data.Char (GeneralCategory(ConnectorPunctuation))
...
data ListaTransformaciones = Matr ((Float,Float),(Float, Float)) ListaTransformaciones | Cero deriving Show
...
aplicarTransformaciones Cero (x,y) = (x,y)
aplicarTransformaciones (Matr ((x12, x14), (x21, x22)) ab) (x,y) = aplicarTransformaciones ab (transformacionLineal ((x12, x22), (x21, x22)) (x, y))


transformacionLineal ((m11, m12), (m21, m22)) (n, m) = ((m11 * n) + (m12 * m), (m21 * n) + (m22 * m))
...
sonIguales Nil (Cons n ns) = False 
sonIguales (Cons n ns) Nil = False 
...
sonEquivalentes a Nil Nil = True 
sonEquivalentes a Nil (Cons n ns) = False 
sonEquivalentes a (Cons n ns) Nil = False 
sonEquivalentes a (Cons n ns ) (Cons g gs) = mod n a == mod g a && sonEquivalentes a ns gs
