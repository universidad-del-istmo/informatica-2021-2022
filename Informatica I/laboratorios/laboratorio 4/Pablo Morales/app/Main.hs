...
--contar (Cons 1(Cons 2 (Cons 3 Nill) 
-- | n= 1 ns =  Cons 2 (Cons 3 Nill)
-- 1 +  contar (Cons 2 (Cons 3 Nill))
-- | n = 2,  ns = (Cons 3 Nill)
-- 1 +  1 + contar (Cons 3 Nill)
-- | n = 3 ns = cons Nill 
-- 1 + 1 + 1 + contar Nill
-- 1 + 1 + 1 + 0 
-- 3
...
data ListaTransformaciones =  Trans ((Float, Float), (Float, Float)) ListaTransformaciones | Nill deriving Show
...
aplicarTransformaciones (Trans n ns) v = aplicarTransformaciones ns (transformacionLineal n v)
...
artmod :: Int -> Lista -> Lista
artmod m Nil = Nil 
artmod m (Cons n ns) = Cons (mod n m) (artmod m ns)


sonEquivalentes :: Int -> Lista -> Lista -> Bool
--sonEquivalentes m Nil Nil = mod Nil m == mod Nil m
